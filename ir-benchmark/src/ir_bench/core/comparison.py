...
def build_comparison(reports: list[dict]) -> dict:
    engines = [
        {
            ...
            "ranking": r["engine"].get("ranking"),
            "keyword_setup": r["engine"].get("keyword_setup"),
            "run_tag": r["engine"].get("run_tag"),
        }
        for r in reports
    ]
            ...
            by_dataset[dataset_id].append(
                {
                    ...
                    "operational": dataset.get("operational", {}),
    ...
    return {
        ...
        "engines": engines,
        "datasets": [{"dataset_id": dataset_id, "rows": by_dataset[dataset_id]} for dataset_id in order],
    }
...
def _best_name(rows: list[dict], group: str, key: str, higher_is_better: bool) -> tuple[str | None, float | None]:
...
def _quality_table(rows: list[dict]) -> list[str]:
    columns = [
        ("ndcg_cut_10", "nDCG@10", 4),
        ("recall_100", "Recall@100", 4),
        ("map", "MAP", 4),
        ("recip_rank", "MRR", 4),
    ]
    bests = {key: _best_name(rows, "metrics", key, True)[1] for key, _, _ in columns}
    out = ["| Engine | " + " | ".join(label for _, label, _ in columns) + " |"]
    out.append("|---|" + "|".join("---" for _ in columns) + "|")
    for row in rows:
        cells = [_cell(_value(row, "metrics", key), bests[key], places) for key, _, places in columns]
        out.append(f"| {row['engine']} | " + " | ".join(cells) + " |")
    return out


def _operational_table(rows: list[dict]) -> list[str]:
    higher = {"ingest_docs_per_sec"}
    columns = [
    ...
    bests = {label: _best_name(rows, group, key, hib)[1] for group, key, label, _, hib in columns}
...
def render_comparison_markdown(comparison: dict) -> str:
    lines: list[str] = ["# Keyword-search engine comparison", ""]
    ...
    lines.append("")
    lines.append("| Engine | Version | Ranking | Keyword setup |")
    lines.append("|---|---|---|---|")
    for engine in comparison["engines"]:
        lines.append(
            f"| {engine['name']} | {engine.get('version') or 'n/a'} | {engine.get('ranking')} | "
            f"{engine.get('keyword_setup')} |"
        )
    lines.append("")
    ...
    for dataset in comparison["datasets"]:
        ...
        ndcg_best_name, ndcg_best = _best_name(rows, "metrics", "ndcg_cut_10", True)
        p50_best_name, p50_best = _best_name(rows, "latency", "p50_ms", False)
        if ndcg_pos and p50_pos:
            lines.append(
