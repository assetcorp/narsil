...
export class RestBackend implements NarsilBackend {
  ...
  private async insertBatched(plan: IndexLoadPlan): Promise<void> {
    ...
    const flush = async (): Promise<void> => {
      ...
      const result = await this.client.insertBatchSerialized(plan.indexName, batch)
  ...
  private async createAndFill(plan: IndexLoadPlan): Promise<void> {
    await this.client.createIndex(plan.indexName, { schema: plan.schema, language: plan.language })
    try {
      await this.insertBatched(plan)
    } catch (err) {
      try {
  ...
  private async loadIndexIfMissing(plan: IndexLoadPlan): Promise<void> {
    const existing = await this.client.listIndexes()
    if (!existing.some(idx => idx.name === plan.indexName)) {
      await this.createAndFill(plan)
    }
