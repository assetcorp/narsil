...
describe('page cursor', () => {
  it('round-trips a score cursor', () => {
    const cursor = { anchor: 'doc-42', score: 4.523, sortKey: null, sortSignature: null, depth: 0, binding }
  ...
  it('round-trips an anchor-only cursor', () => {
    const cursor = { anchor: 'doc-42', score: null, sortKey: null, sortSignature: null, depth: 0, binding }
    expect(decodePageCursor(encodePageCursor(cursor))).toEqual(cursor)
  })

  it('round-trips the depth a vector search seeks to', () => {
    const cursor = { anchor: 'doc-42', score: 0.5, sortKey: null, sortSignature: null, depth: 250, binding }
    expect(decodePageCursor(encodePageCursor(cursor))).toEqual(cursor)
  })

  it('reads a cursor carrying no depth as a first page', () => {
    expect(decodePageCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, q: binding })).depth).toBe(0)
  })

  it('rejects a depth that is not a positive integer', () => {
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, d: 0, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, d: -20, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, d: 12.5, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, d: '40', q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, d: Number.MAX_SAFE_INTEGER + 2, q: binding }))
  })

  it('rejects invalid base64, invalid JSON, and a non-object payload', () => {
    expectInvalidCursor('!!!not-base64!!!')
    expectInvalidCursor(Buffer.from('{"v":4,"a"').toString('base64'))
    expectInvalidCursor(encodeRaw([{ v: 4, a: 'doc-1' }]))
  })

  it('rejects any version other than 4', () => {
    expectInvalidCursor(encodeRaw({ v: 3, a: 'doc-1', s: 1, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 5, a: 'doc-1', s: 1, q: binding }))
    expectInvalidCursor(encodeRaw({ a: 'doc-1', s: 1, q: binding }))
  })

  it('rejects a missing or malformed binding', () => {
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1 }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, q: 'AB12CD34' }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, q: 'ab12cd3' }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, q: 12345678 }))
  })

  it('rejects a missing, empty, or oversized anchor', () => {
    expectInvalidCursor(encodeRaw({ v: 4, s: 1, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: '', s: 1, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'x'.repeat(513), s: 1, q: binding }))
  })
  ...
  it('accepts an anchor of 512 supplementary characters, which exceed 512 UTF-16 units', () => {
    ...
    const decoded = decodePageCursor(encodeRaw({ v: 4, a: anchor, s: 1, q: binding }))
    expect(decoded.anchor).toBe(anchor)
  })

  it('rejects a non-finite score', () => {
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 'high', q: binding }))
    expectInvalidCursor(Buffer.from(`{"v":4,"a":"doc-1","s":1e999,"q":"${binding}"}`).toString('base64'))
  })

  it('rejects a cursor carrying both a score and a sort key', () => {
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', s: 1, k: ['x'], o: '[["f","asc"]]', q: binding }))
  })

  it('rejects a sort key without its sort order, and the reverse', () => {
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', k: ['x'], q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', o: '[["f","asc"]]', q: binding }))
  })

  it('rejects a sort key holding more than 8 values or a disallowed value', () => {
    const order = '[["f","asc"]]'
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', k: Array.from({ length: 9 }, () => 1), o: order, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', k: [{ nested: true }], o: order, q: binding }))
    expectInvalidCursor(encodeRaw({ v: 4, a: 'doc-1', k: ['x'.repeat(513)], o: order, q: binding }))
    expectInvalidCursor(
      Buffer.from(`{"v":4,"a":"doc-1","k":[1e999],"o":"[[\\"f\\",\\"asc\\"]]","q":"${binding}"}`).toString('base64'),
    )
  })
