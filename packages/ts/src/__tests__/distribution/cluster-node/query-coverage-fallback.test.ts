...
describe('a cluster search on a node that refuses partial results', () => {
  let coordinator: ClusterCoordinator
  let network: InMemoryNetwork
  let transport: NodeTransport
  let node: ClusterNode
  ...
  it('refuses the search while no replica is active yet', async () => {
    const table = makeAllocationTable(
      [
        [0, makeAssignment({ state: 'INITIALISING' })],
        [1, makeAssignment({ state: 'INITIALISING' })],
    ...
    const thrown = await node.query('shop', { term: 'portable', limit: DOCUMENT_TOTAL }).catch((err: unknown) => err)

    expect(thrown).toBeInstanceOf(NarsilError)
    expect((thrown as NarsilError).code).toBe(ErrorCodes.QUERY_PARTIAL_FAILURE)
  }, 30_000)

  it('answers from the local copy for an index the cluster has never allocated', async () => {
    expect(await coordinator.getAllocation('shop')).toBeNull()
