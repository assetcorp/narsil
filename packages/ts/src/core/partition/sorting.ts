...
function ensureColumns(state: PartitionState): void {
  if (state.sortColumns === null) {
    state.sortColumns = createSortColumnSet(state.docStore)
  }
}

export function sortedPageOf(state: PartitionState, request: SortPageRequest): SortedPageEntry[] {
  ensureColumns(state)
...
export function recordSortValues(state: PartitionState, internalId: number, document: Record<string, unknown>): void {
  state.sortColumns?.record(internalId, document)
}

export function forgetSortValues(state: PartitionState, internalId: number | undefined): void {
  if (internalId === undefined) return
  state.sortColumns?.forget(internalId)
}

export function refreshSortColumns(state: PartitionState): void {
  state.sortColumns?.refresh()
}

export function sortColumnBytes(state: PartitionState): number {
