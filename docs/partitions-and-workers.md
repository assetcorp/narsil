...
On Node.js, Bun, and Deno an embedded engine divides `workers.count` between a keyword pool of `Math.ceil(count / 2)` threads and a vector search pool of the rest. Where you name no count, the engine takes one thread fewer than the cores the runtime reports and keeps that total between 2 and 8, which leaves the keyword pool between 1 and 4 threads. Every thread of the keyword pool holds a copy of each large index, so keyword search answers on those threads without a setting. The HTTP server holds a copy on every thread of the budget and receives requests on those threads, and they search a vector copy the engine shares with them; see [Request threads](#request-threads). An index gains its copies once it holds `promotionThreshold` documents, which defaults to 1,000, because below that a query finishes sooner than the hop to a worker thread takes. A browser page holds no copies, and `workers.enabled: false` holds a Node.js process to one thread, which also leaves the vector search pool absent. The API stays identical whether or not an index holds copies.
...
The engine sends a whole query to an idle copy, so concurrent queries spread across the copies. Where no other query is in flight and the index holds at most 50,000 documents, the main copy answers the query, because crossing to a worker thread and back adds time that no other query is there to use. On SciFact one search at a time then takes 0.10 ms, the same as it takes with copies off. Above 50,000 documents every query goes to a copy, so the main thread stays free during a search of about 0.9 ms or longer. Once every copy is busy, the main copy answers one query per turn of the event loop and the rest queue on the least busy copy. The main thread therefore adds its own capacity while it still processes the replies the copies send back. That rule keeps a lone query fast in a process whose main thread executes your own code between queries, because the hop to a worker thread would only add latency there. In a process whose main thread also serves requests, such as an HTTP server, each request reaches the engine on its own turn of the event loop while no other query is in flight, so every query would answer on the main copy while the copies stay idle. Under `workers.mainCopyQueries: 'none'`, every query on an index that holds copies goes to one of them, so the main thread keeps serving requests. A server created with `createServer` sets that value on an engine that leaves the field unset, and it also receives each query on the request thread that holds a copy, so the main copy under a server sees only the queries on indexes without copies. On SciFact at 16 clients on an 11 core machine, one copy alone answered 8,656 queries per second against 9,635 on one thread, and one copy plus the main copy answered 13,072. A query that names several partitions may split across idle copies, each answering its own partitions, before the engine merges the answers. On SciFact, a query answers with the same hits, scores, count, and facets whether copies are on or off. At 16 concurrent clients the engine answers about three times as many queries per second with copies on as it does on one thread.

### Request threads

The HTTP server receives every connection on the workers that hold the copies, and the engine calls each such worker a request thread. The main thread accepts a connection and moves it to the next request thread in turn, so a client that keeps a connection open stays on one thread for as long as the connection lasts. A request thread answers a query on an index whose copy it holds from that copy, and it searches a vector field from a frozen copy of the field that every thread shares in memory, so a hybrid query fuses, pages, and replies without the main thread. It sends every write, every admin request, and a query on an index whose copy it lacks to the main thread, which answers as it does for its own callers. Three kinds of query on a held copy still need the main thread. A query whose `vector.text` an embedding adapter must embed has the main thread embed the text, and it then answers from the copy. Every search goes to the main thread whole while a plugin registers a `beforeSearch` or `afterSearch` hook, because a hook is a function that only the main thread holds. A query naming a vector field whose graph the engine has yet to build goes whole as well, until the frozen copy of that field reaches the threads. The server starts as many request threads as `workers.count`, one alone on a machine with fewer than three cores, and every one of them holds a copy of each scaled-out index, so no separate vector search pool runs under the server. `NarsilServer.requestThreadCount` reports the count, and the `maxConcurrentRequests` limit counts the requests in flight on every thread together. When a request thread dies, the main thread stops sending it connections, the survivors keep answering, and the replacement worker takes requests again once its copies have loaded. An engine created with `workers.enabled: false`, a runtime without worker threads, and a cluster node's engine each leave the main thread answering every request itself.

### Writes and the copies

A write returns once the main copy holds it, and the copies apply it afterwards through a queue the write does not wait on, so a query that a copy answers can come back without a write that returned before it. The copies apply writes in the order the writes returned. Pass `wait: true` on an `insert`, `update`, `remove`, or batch call to make that write return only once every copy holding the index has applied it, or call `waitForWrites(indexName)` after a run of writes to resolve once the copies have applied every write that returned before the call. Over HTTP the same option travels as `options.wait` in a write body and as `?wait=true` on `DELETE`, and `POST /indexes/{name}/_wait-for-writes` waits for the run. An index that holds no copies answers every query from the main copy, so a write there is visible as soon as it returns. A write with `wait: true` that arrives while a rebalance reshapes the index returns once the reshape has replayed it from the write-ahead queue and every copy has applied it. Elasticsearch, Meilisearch, and Qdrant each return from a write before it is searchable and let the caller opt into waiting, which puts the cost on the caller who needs it.
...
Segments accumulate as you keep inserting, so a partition holding eight of them compacts them into one in the background while queries keep answering. Once no write has reached the index for one second, the engine freezes the live tail of each partition into a shared segment where the tail holds 64 documents or more. A worker then merges every segment of the partition into one, on every copy and on the main copy alike, so that a copy answers a query about as fast as the main copy does alone. The engine also freezes a tail that reaches 5,000 documents during ingest, which holds the time the main thread spends encoding a tail to about 120 ms on SciFact documents. The freeze keeps memory per copy small whatever the write pattern, because every copy holds a private structure for each document inserted on its own until the freeze moves that document into shared memory. Every keyword copy and every vector search worker also starts under a 24 MB cap on its young generation, which is the space V8 gives to short-lived objects in that thread. However, the old space of a copy and the shared segments it attaches grow outside that cap. All of this needs no configuration, and `getMemoryStats()` reports what each worker holds.
