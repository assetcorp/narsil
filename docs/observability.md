...
The engine emits `heapPressure` once the process spends nine tenths of the heap that V8 reports, measured during a write to an index, a restore, a reopen, or the recovery of persisted indexes at start-up, where the event names the largest index. The engine compares the used bytes with the headroom that V8 reports, because V8 keeps part of the raw limit back and an allocation fails before the used bytes reach that limit.

That reserve is about 192 MB whatever the limit you set, so V8 reports a limit 192 MB above your figure and a small heap dies well short of nine tenths. Measured on Node 24.16 on Apple silicon, Node ended the process at 0.67 of the reported heap under `--max-old-space-size=256`, at 0.72 under 512, at 0.84 under 1024, and at 0.91 under 2048. Treat the event as a warning for a process whose limit is 2 GB or above, which covers the default limit on a large host and the `--max-old-space-size-percentage=75` images above. Below that, read `process.memoryUsage().heapUsed` against the limit you set and raise your own alarm.
