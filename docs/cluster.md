...
A query that a data node refuses, such as one with a malformed filter, fails with the error that the node raises, since every node would raise the same one. Each data node counts facet values over its own partitions, and the coordinator merges them and returns up to each field's `limit`, 10 values by default, highest count first. The cluster message format has room for one value count per facet field and for neither ranges nor a sort order, so a cluster search throws `CLUSTER_OPERATION_UNSUPPORTED` for a facet with `ranges` or with `sort: 'asc'`.
