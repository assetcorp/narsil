...
`checkpoint`, `open`, `close`, `getMemoryStats`, `on`, and `off` reach the local engine of the node you call them on, because durability, index memory, process memory, and engine events are per-node facts. Ask each node for its own. When you call `close`, the node releases only its own copy of the index, and it reopens the copy when it receives a routed read or write; see [Index lifecycle](persistence-and-durability.md#index-lifecycle).
