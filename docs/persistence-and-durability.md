...
Only one engine may write a durability directory at a time. Before recovery, the engine writes a `.narsil.lock` file into the directory with its process id, its thread id, the time its process started, and a random token. `createNarsil` fails with `CONFIG_INVALID` while another running process, another thread, or another engine in the same process has the directory open, whether through the same path, another spelling of it, or a symbolic link. The engine deletes the file on `shutdown()`, and also when `createNarsil` fails after writing it, so that a retry in the same process can open the directory. A process that crashes leaves the file behind, and because the file then records a process that no longer runs, the next engine replaces it. Where another program later receives that process id, as after a reboot, `createNarsil` refuses the directory, and the error gives the path of the lock file to delete. The engine reads the token again before it acknowledges each `sync` write and once a second, so writes fail with `PERSISTENCE_SAVE_FAILED` once another engine replaces the file.
