...
The engine ranks every vector in a field, whatever its similarity score, so `count` holds the number of vectors that pass the query's filter and its `similarity` floor. Where a query sets neither of them, that number is the count of every vector in the field. Where the query sets a filter, it is the number of documents that pass the filter and have a vector.

Check `countExact` to find out whether that number is exact. It is true wherever the engine compares the query with every vector that passes, which the engine does for a field below the promotion threshold, for a filter narrow enough that the engine skips the graph, and for any query without a `similarity` floor. It is false where a query sets a floor on a field that holds a graph, because the engine then compares the query with only a fraction of the field's vectors. `count` then holds the number of vectors that the engine fetches, which is at or below the true total.

For each page of a vector search, the engine fetches every result down to the depth in the cursor, plus one more page, so each deeper page takes more work than the page before it. Where that depth is beyond the 10,000-result window, the engine raises `SEARCH_RESULT_WINDOW_EXCEEDED`. Set a filter or a `similarity` floor to keep a long run of pages inside that window.
...
Narsil does the same work through WebAssembly in a browser, on any other platform, and wherever the runtime cannot load the core. Narsil prints one warning on each thread that falls back, naming the reason. To hold a process to the WebAssembly search, set `NARSIL_SEARCH_BACKEND=wasm` before you start it. That variable takes `native` or `wasm`, in upper case or lower, while `createNarsil` fails with `CONFIG_INVALID` for any other value. To load a core from a path of your own, such as one that you compiled, set `NARSIL_NATIVE_CORE_PATH` to the file. Set `NARSIL_REQUIRE_NATIVE_CORE=1` to hold a process to the core alone. `createNarsil` then loads the core as it starts and fails with `CONFIG_INVALID` where no core loads, so a deployment check that waits for the engine to start catches a host without the core. To find out which path a server takes, call `GET /version` and read its `vectorSearch` field, which holds `native` or `wasm`.
