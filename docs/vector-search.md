...
Narsil does the same work through WebAssembly in a browser, on any other platform, and wherever the runtime cannot load the core. To hold a process to the WebAssembly search, set `NARSIL_SEARCH_BACKEND=wasm` before you start it. To load a core from a path of your own, such as one that you compiled, set `NARSIL_NATIVE_CORE_PATH` to the file. Set `NARSIL_REQUIRE_NATIVE_CORE=1` to hold a process to the core alone, and Narsil then raises a `NarsilError` with code `CONFIG_INVALID` the first time that it needs a core and has none to load. To find out which path a server takes, call `GET /version` and read its `vectorSearch` field, which holds `native` or `wasm`.
