...
| `count` | `number` | CPU cores minus one, clamped between 2 and 8 | Sets the thread budget that the keyword copies and the vector search pool share, half each in an embedded engine, so a budget of 4 gives 2 copies and 2 vector search workers. The engine caps a value that you set at 31, which is how many scratch slots the shared vector arena reserves. The HTTP server holds a copy on every thread of the budget and receives requests on them; see [Request threads](partitions-and-workers.md#request-threads). |
...
| `bootstrapModule` | `string` | none | Names a module every worker imports at startup so that the worker registers the languages, tokenizers, and stop word sets your indexes name. Give an absolute path or a URL, such as `new URL('./bootstrap.js', import.meta.url).href`, because a worker resolves a relative specifier against Narsil's own directory; a relative value raises `CONFIG_INVALID`. See [Worker copies](partitions-and-workers.md#worker-copies). |
