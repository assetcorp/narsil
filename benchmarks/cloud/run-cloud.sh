#!/usr/bin/env bash
#
# Provision a fixed, disclosed VM on the selected cloud, run both Narsil
# benchmark suites on it, copy the results back into this repository, and tear
# the VM down. Absolute performance numbers only reproduce on one named machine,
# so every provider defaults to a dedicated-vCPU instance whose CPU is fixed and
