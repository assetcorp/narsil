...
cmd_run() {
  ...
  for v in BENCH_INPROCESS_TIERS BENCH_SERVER_ENGINES \
    BENCH_BEST_CONFIG BENCH_THROUGHPUT_PASSES BENCH_DATASETS BENCH_MEM_CAP BENCH_JVM_HEAP; do
    if [ -n "${!v:-}" ]; then
...
Common env:
  ...
  BENCH_BEST_CONFIG, BENCH_THROUGHPUT_PASSES, BENCH_DATASETS, BENCH_MEM_CAP, BENCH_JVM_HEAP.
