...
# A run id and an engine name both become a single directory or file segment under
# the results tree, so they must not start a hidden entry, climb out with a dot
# entry, carry a path separator, or smuggle a null byte. The leading-alphanumeric
# rule rejects '.', '..', and option-like leading dashes; the body allows only the
# characters a timestamp id and an engine name actually use.
...
def _validate_segment(value: str, kind: str) -> str:
    if not isinstance(value, str) or not _SEGMENT.match(value):
        raise ValueError(
            f"invalid {kind} {value!r}: expected 1-64 characters of letters, digits, "
            "dot, dash, or underscore, not starting with a dot or dash"
        )
    return value
...
def validate_engine_name(name: str) -> str:
    return _validate_segment(name, "engine name")
...
def resolve_run_id_for_read(results_dir: Path, explicit: str | None) -> str:
    raw = (explicit or os.environ.get(RUN_ID_ENV) or "").strip()
    if raw:
    ...
    if latest is None:
        raise ValueError(
            f"no benchmark runs found under {runs_root(results_dir)}; run the harness "
            "before aggregating, or pass --run-id"
        )
    return latest


def write_run_manifest(results_dir: Path, run_id: str, environment: dict) -> Path:
