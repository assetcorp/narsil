...
def _build_cell(engine: dict) -> str:
    ...
    if commit:
        return f"{commit[:12]}{' (unreleased)' if build.get('dirty') else ''}"
...
def _engine_table(comparison: dict) -> list[str]:
    lines = ["## Engines and tracks", "", "| Engine | Version | Build | Tracks |", "| --- | --- | --- | --- |"]
...
def _dataset_section(track: str, dataset: dict) -> list[str]:
    rows = dataset["rows"]
...
def render_comparison_markdown(comparison: dict) -> str:
    ...
    lines.extend(_run_conditions(comparison))
    lines.extend(_engine_table(comparison))
    for track in comparison["tracks"]:
        lines.append(f"## {_TRACK_TITLES.get(track['track'], track['track'])}")
        lines.append("")
        for dataset in track["datasets"]:
            lines.extend(_dataset_section(track["track"], dataset))
    return "\n".join(lines)
