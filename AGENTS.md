...
- Put every tuning value, meaning a threshold, an interval, a size, or a retry budget, in its directory's `constants.ts`. Leave a byte layout, a wire tag, and a format version beside the code that reads them. Name a constant `DEFAULT_X` where a config field overrides it, `MAX_X` or `MIN_X` where the engine throws a `NarsilError` beyond it, and a plain name where nothing overrides it, ending a duration in `_MS` and a size in `_BYTES`. A `constants.ts` holds literals alone and imports nothing at runtime beyond another `constants.ts`, so a bundler drops what a caller never reads and no platform code crosses through one. The 400-line limit skips a `constants.ts`, which stays one file however many values it holds.
