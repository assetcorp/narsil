...
- Keep every file under 400 lines. Measure a file when you touch it, and split it before your change pushes it over. Turn a module that outgrows one file into a directory with an `index.ts` that re-exports it, and drop the module prefix from each filename inside. Copy `src/core/partition/` or `src/engine/mutations/`.
...
- **`serialization/envelope.ts` stamps `VERSION` from `src/index.ts` into every `.nrsl` file.** It parses that constant into the three engine-version bytes the format defines, so the constant forms part of the file contents.
...
- **Root `pnpm test` reaches past the library.** `nx run-many -t test` includes the browser and server-app examples and the benchmarks project. Name the target you want.
